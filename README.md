...
<p align="center"><em>The test contract between languages.</em></p>
...
Language-independent test vectors. An SDK may call itself compatible with a spec version only if it passes that version’s mandatory suite. Vectors compare normalized JSON results — fields, enum identifiers, error codes, event ordering, retry decisions — never internal classes.
